...
function RangeBarChart({ title, items, formatValue = (value) => String(value) }) {
        ...
        {items.map((item) => {
          ...
          return (
            <div key={item.key ?? item.label} className="grid grid-cols-[7rem_1fr] gap-3 text-xs sm:grid-cols-[9rem_1fr_6rem] sm:items-center">
              ...
              <strong className="col-span-2 text-right text-[#162338] sm:col-span-1">{formatValue(value)}</strong>
            </div>
          );
        })}
      </div>
...
function buildNightDutyRangeReportLines(reports, rangeStart, rangeEnd) {
  ...
  const lines = [
    ...
    { text: `Average nightly occupancy: ${formatAverage(analysis.averageOccupancy)}`, bold: true },
  ...
  lines.push({
    ...
    items: analysis.occupancyByFloor.map((floor) => ({
      ...
      displayValue: String(floor.totalOccupants),
    })),
    spaceBefore: 8,
    spaceAfter: 8,
  });
  ...
  lines.push({
    ...
    items: analysis.incomeByOutlet.map((outlet) => ({
      ...
      displayValue: formatAmount(outlet.grandRevenueTotal),
    })),
    spaceBefore: 8,
    spaceAfter: 8,
  });
  ...
  lines.push("Generator service totals:");
  if (analysis.generatorServiceTotals.length === 0) lines.push("- Nil");
  analysis.generatorServiceTotals.forEach((entry) => {
...
function printNightDutyRangeReport(reports, rangeStart, rangeEnd) {
  ...
  const notesMarkup = analysis.departmentalNotes.length > 0
    ...
    : "<p>Nil</p>";
  const generatorTotals = analysis.generatorServiceTotals.length > 0
    ? analysis.generatorServiceTotals.map((entry) => `${escapeHtml(entry.name)}: ${escapeHtml(formatTotalMinutes(entry.totalMinutes))}`).join("<br>")
    : "Nil";
  ...
  const chartMarkup = (title, items, formatter) => {
    const maximum = Math.max(...items.map((item) => Number(item.value) || 0), 1);
    return `<section class="chart"><h3>${escapeHtml(title)}</h3>${items.map((item) => `<div class="bar-row"><span>${escapeHtml(item.label)}</span><div class="bar-track"><i style="width:${Math.max(((Number(item.value) || 0) / maximum) * 100, Number(item.value) > 0 ? 1 : 0)}%"></i></div><strong>${escapeHtml(formatter(item.value))}</strong></div>`).join("")}</section>`;
  };
  ...
  reportWindow.document.write(`
    <!doctype html><html><head><title>Sunshine Hotel Full Night Duty Report</title>
      <style>
        ...
        .bar-track i { background: #8a6923; display: block; height: 100%; }
...
export default function NightDutyPanel({
  ...
  async function backupReportRange() {
    ...
    try {
      ...
      setFeedback({
        type: "success",
        message: `${result.archivedRecords} Night Duty report(s) backed up in Cloudflare D1. ${result.coveredDates} selected date(s) are now marked as checked.`,
      });
    } catch (error) {
      setFeedback({ type: "error", message: error.message });
    } finally {
            ...
            <div>
              ...
              <p className={`mt-2 text-xs font-semibold ${hasCloudflareArchiveConfig ? "text-emerald-700" : "text-amber-700"}`}>
                {hasCloudflareArchiveConfig
                  ...
                  : "Cloudflare D1 is not active yet. Reports currently use Firebase only."}
              </p>
            </div>
            ...
            {rangeReportLoaded ? (
              <div className="mt-5">
                <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-5">
                  ...
                  <div className="rounded-xl bg-slate-50 p-4 text-sm">Average occupancy<br /><strong className="text-xl text-[#162338]">{formatAverage(rangeAnalytics.averageOccupancy)}</strong></div>
                ...
                {rangeReportData.length > 0 ? (
                  <div className="mt-6 space-y-6">
                    <div className="grid gap-4 xl:grid-cols-2">
                      ...
                      <RangeBarChart
                        title="Grand Revenue by section"
                        items={rangeAnalytics.incomeByOutlet.map((outlet) => ({ key: outlet.key, label: outlet.label, value: outlet.grandRevenueTotal }))}
