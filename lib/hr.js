...
export function getEmploymentStartDateKey(profile = {}) {
  return (
    profile.employmentStartDate ||
    toDateKey(profile.createdAt) ||
    toDateKey(profile.updatedAt) ||
    ""
  );
}
...
export function getLeaveEligibilityDetails(profile = {}, now = new Date()) {
  const todayKey = getHotelDateKey(now);
  const employmentStartDateKey = getEmploymentStartDateKey(profile);
  ...
  const leaveRecords = normalizeLeaveRecords(profile.leaveRecords ?? []);
...
export function getLeaveDashboardSummary(profile = {}, now = new Date()) {
...
export function buildPayrollBreakdown(profile = {}, now = new Date()) {
  const payrollMonthKey = toMonthKey(profile.payrollMonthKey) || getHotelDateKey(now).slice(0, 7);
  const monthlySalary = Math.max(toNumber(profile.monthlySalary), 0);
  const absenceDays = Math.max(toNumber(profile.absenceDays), 0);
  const lateCount = Math.max(toNumber(profile.lateCount), 0);
