...
function normalizePdfLine(line) {
  ...
  if (line?.type === "table") {
    ...
    return {
      ...
      fontSize: Math.min(Math.max(Number(line.fontSize) || 8.5, 8), 11),
      rowHeight: Math.min(Math.max(Number(line.rowHeight) || 21, 19), 30),
...
function wrapPdfLine(line) {
  ...
  if (
    ...
    normalizedLine.type === "groupedBarChart"
  ) {
...
function buildPdfPageContent(title, bodyLines = [], pageNumber = 1, pageCount = 1) {
  ...
  bodyLines.forEach((line) => {
    ...
    if (normalizedLine.type === "lineChart") {
      ...
      [0, 0.25, 0.5, 0.75, 1].forEach((ratio) => {
        ...
        commands.push(
          ...
          `q 0.4 0.45 0.52 rg BT /F1 6 Tf 48 ${(y - 2).toFixed(2)} Td (${escapePdfText(formatPdfScaleValue(maximum * ratio, normalizedLine.valueSuffix))}) Tj ET Q`,
        );
      });
      ...
      normalizedLine.series.forEach((series, seriesIndex) => {
        ...
        commands.push(
          ...
          `BT /F1 7 Tf ${legendX + 11} ${(legendY + 1).toFixed(2)} Td (${escapePdfText(series.label)}) Tj ET`,
        );
      });
    ...
    if (normalizedLine.type === "groupedBarChart") {
      commands.push(
        `BT /F2 12 Tf 48 ${currentY} Td (${escapePdfText(normalizedLine.title)}) Tj ET`,
      );
      currentY -= 24;
      const chartLeft = 72;
      ...
      const barWidth = Math.max(
        Math.min((groupWidth * 0.78) / Math.max(normalizedLine.series.length, 1) - gap, 13),
        1.2,
      );
      const colors = [
        "0.65 0.49 0.18",
      ...
      [0, 0.25, 0.5, 0.75, 1].forEach((ratio) => {
        ...
        commands.push(
          `q 0.85 0.89 0.93 RG 0.35 w ${chartLeft} ${y.toFixed(2)} m ${(chartLeft + chartWidth)} ${y.toFixed(2)} l S Q`,
          `q 0.4 0.45 0.52 rg BT /F1 7 Tf 48 ${(y - 2).toFixed(2)} Td (${escapePdfText(formatPdfScaleValue(maximum * ratio))}) Tj ET Q`,
        );
      });

      normalizedLine.labels.forEach((label, labelIndex) => {
        const groupStart = chartLeft + labelIndex * groupWidth + groupWidth * 0.11;
        normalizedLine.series.forEach((series, seriesIndex) => {
          const value = Number(series.values[labelIndex]) || 0;
          const height = (value / maximum) * chartHeight;
          commands.push(
            `q ${colors[seriesIndex % colors.length]} rg ${(groupStart + seriesIndex * (barWidth + gap)).toFixed(2)} ${chartBottom.toFixed(2)} ${barWidth.toFixed(2)} ${height.toFixed(2)} re f Q`,
          );
        });
      });
      ...
      normalizedLine.labels.forEach((label, index) => {
        if (index % labelStep === 0 || index === normalizedLine.labels.length - 1) {
          ...
          commands.push(
            `BT /F1 7 Tf ${(x - 9).toFixed(2)} ${(chartBottom - 10).toFixed(2)} Td (${escapePdfText(label.slice(5))}) Tj ET`,
          );
        }
      });
      normalizedLine.series.forEach((series, seriesIndex) => {
        ...
        commands.push(
          `q ${colors[seriesIndex % colors.length]} rg ${legendX} ${legendY.toFixed(2)} 8 8 re f Q`,
          `BT /F1 8 Tf ${legendX + 12} ${(legendY + 1).toFixed(2)} Td (${escapePdfText(series.label)}) Tj ET`,
        );
      });
