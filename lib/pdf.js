...
function formatPdfScaleValue(value, suffix = "") {
  const number = Number(value) || 0;
  if (suffix) return `${Math.round(number)}${suffix}`;
  if (Math.abs(number) >= 1000000) return `${(number / 1000000).toFixed(1)}m`;
  if (Math.abs(number) >= 1000) return `${(number / 1000).toFixed(0)}k`;
  return String(Math.round(number));
}

function formatPdfChartAxisLabel(value) {
  const label = String(value ?? "").trim();
  return /^\d{4}-\d{2}-\d{2}$/.test(label) ? label.slice(5) : label;
}

function hexToPdfColor(value, fallback) {
  const match = String(value ?? "").trim().match(/^#?([0-9a-f]{6})$/i);
  if (!match) return fallback;
  const hex = match[1];
  return [0, 2, 4]
    .map((index) => (Number.parseInt(hex.slice(index, index + 2), 16) / 255).toFixed(3))
    .join(" ");
}
...
function normalizePdfLine(line) {
  ...
  if (line?.type === "lineChart") {
    ...
    return {
      ...
      title: String(line.title ?? ""),
      labels,
      xAxisLabel: String(line.xAxisLabel ?? "Activity date").slice(0, 40),
      yAxisLabel: String(line.yAxisLabel ?? "Value").slice(0, 40),
      series: (Array.isArray(line.series) ? line.series : []).slice(0, 12).map((series) => ({
  ...
  if (line?.type === "stackedBarChart") {
    ...
    return {
      ...
      labels,
      xAxisLabel: String(line.xAxisLabel ?? "Activity date").slice(0, 40),
      yAxisLabel: String(line.yAxisLabel ?? "Revenue amount").slice(0, 40),
      series: (Array.isArray(line.series) ? line.series : []).slice(0, 8).map((series) => ({
        label: String(series?.label ?? "Series").slice(0, 28),
        color: String(series?.color ?? ""),
        values: labels.map((_, index) => Math.max(Number(series?.values?.[index]) || 0, 0)),
      })),
  ...
  if (line?.type === "table") {
    ...
    return {
      ...
      fontSize: Math.min(Math.max(Number(line.fontSize) || 9.5, 9), 12),
      rowHeight: Math.min(Math.max(Number(line.rowHeight) || 23, 21), 32),
  ...
  return {
    ...
    fontSize: Math.min(Math.max(Number(line?.fontSize) || 12, 9), 19),
...
function wrapPdfLine(line) {
  ...
  if (
    normalizedLine.type === "barChart" ||
    normalizedLine.type === "lineChart" ||
    normalizedLine.type === "stackedBarChart"
  ) {
...
function getPdfLineHeight(line) {
  ...
  if (line.type === "lineChart") {
    return line.spaceBefore + 223 + Math.ceil(line.series.length / 4) * 12 + line.spaceAfter;
  }

  if (line.type === "stackedBarChart") {
    return line.spaceBefore + 243 + Math.ceil(line.series.length / 4) * 13 + line.spaceAfter;
  }
...
const MAX_PAGE_BODY_HEIGHT = 680;
  ...
  wrappedLines.forEach((line, index) => {
    ...
    const nextLineHeight = line.keepWithNext && wrappedLines[index + 1]
      ? getPdfLineHeight(wrappedLines[index + 1]) + (
          line.sectionHeading && wrappedLines[index + 2]
            ? getPdfLineHeight(wrappedLines[index + 2]) + 50
            : 0
        )
      : 0;
...
function buildPdfPageContent(title, bodyLines = [], pageNumber = 1, pageCount = 1) {
  ...
  bodyLines.forEach((line) => {
    ...
    if (normalizedLine.type === "barChart") {
      commands.push(
        `BT /F2 13 Tf 48 ${currentY} Td (${escapePdfText(normalizedLine.title)}) Tj ET`,
      );
      currentY -= 24;
      ...
      normalizedLine.items.forEach((item, itemIndex) => {
        ...
        commands.push(
          `BT /F1 9 Tf 48 ${currentY} Td (${escapePdfText(item.label)}) Tj ET`,
          ...
          `BT /F2 9 Tf 450 ${currentY} Td (${escapePdfText(item.displayValue)}) Tj ET`,
        );
        currentY -= 23;
      });
    ...
    if (normalizedLine.type === "lineChart") {
      ...
      [0, 0.25, 0.5, 0.75, 1].forEach((ratio) => {
        ...
        commands.push(
          ...
          `q 0.4 0.45 0.52 rg BT /F1 8 Tf 48 ${(y - 2).toFixed(2)} Td (${escapePdfText(formatPdfScaleValue(maximum * ratio, normalizedLine.valueSuffix))}) Tj ET Q`,
        );
      });
      ...
      normalizedLine.series.forEach((series, seriesIndex) => {
        const color = hexToPdfColor(series.color, colors[seriesIndex % colors.length]);
        if (series.values.length > 0) {
          const path = series.values.map((value, index) => `${pointX(index).toFixed(2)} ${pointY(value).toFixed(2)} ${index === 0 ? "m" : "l"}`).join(" ");
          commands.push(`q ${color} RG 1.15 w ${path} S Q`);
          series.values.forEach((value, index) => {
            commands.push(`q ${color} rg ${(pointX(index) - 0.7).toFixed(2)} ${(pointY(value) - 0.7).toFixed(2)} 1.4 1.4 re f Q`);
          });
        }
        const legendColumn = seriesIndex % 4;
        const legendRow = Math.floor(seriesIndex / 4);
        const legendX = 62 + legendColumn * 125;
        const legendY = chartBottom - 36 - legendRow * 12;
        commands.push(
          `q ${color} rg ${legendX} ${legendY.toFixed(2)} 7 7 re f Q`,
          `BT /F1 9 Tf ${legendX + 11} ${(legendY + 1).toFixed(2)} Td (${escapePdfText(series.label)}) Tj ET`,
        );
      });
      ...
      normalizedLine.labels.forEach((label, index) => {
        if (index % labelStep === 0 || index === normalizedLine.labels.length - 1) {
          commands.push(
            `BT /F1 8 Tf ${(pointX(index) - 9).toFixed(2)} ${(chartBottom - 8).toFixed(2)} Td (${escapePdfText(formatPdfChartAxisLabel(label))}) Tj ET`,
          );
        }
      });

      commands.push(
        `BT /F2 8 Tf 270 ${(chartBottom - 22).toFixed(2)} Td (${escapePdfText(normalizedLine.xAxisLabel)}) Tj ET`,
        `BT /F2 8 Tf 0 1 -1 0 30 ${(chartBottom + 42).toFixed(2)} Tm (${escapePdfText(normalizedLine.yAxisLabel)}) Tj ET`,
      );

      currentY = chartBottom - 43 - Math.ceil(normalizedLine.series.length / 4) * 12 - normalizedLine.spaceAfter;
      return;
    }

    if (normalizedLine.type === "stackedBarChart") {
      commands.push(
        `BT /F2 13 Tf 48 ${currentY} Td (${escapePdfText(normalizedLine.title)}) Tj ET`,
      );
      currentY -= 24;
      const chartLeft = 72;
      const chartBottom = currentY - 150;
      const chartWidth = 470;
      const chartHeight = 140;
      const maximum = Math.max(...normalizedLine.labels.map((_, labelIndex) =>
        normalizedLine.series.reduce(
          (total, series) => total + Math.max(Number(series.values[labelIndex]) || 0, 0),
          0,
        )
      ), 1);
      const groupWidth = chartWidth / Math.max(normalizedLine.labels.length, 1);
      const barWidth = Math.max(Math.min(groupWidth * 0.62, 24), 4);
      const colors = [
        "0.65 0.49 0.18",
        "0.09 0.14 0.22",
        "0.06 0.46 0.43",
      ...
      [0, 0.25, 0.5, 0.75, 1].forEach((ratio) => {
        ...
        commands.push(
          `q 0.85 0.89 0.93 RG 0.35 w ${chartLeft} ${y.toFixed(2)} m ${(chartLeft + chartWidth)} ${y.toFixed(2)} l S Q`,
          `q 0.4 0.45 0.52 rg BT /F1 8 Tf 48 ${(y - 2).toFixed(2)} Td (${escapePdfText(formatPdfScaleValue(maximum * ratio))}) Tj ET Q`,
        );
      });

      normalizedLine.labels.forEach((label, labelIndex) => {
        const x = chartLeft + labelIndex * groupWidth + (groupWidth - barWidth) / 2;
        let cumulative = 0;
        const dayTotal = normalizedLine.series.reduce(
          (total, series) => total + Math.max(Number(series.values[labelIndex]) || 0, 0),
          0,
        );
        normalizedLine.series.forEach((series, seriesIndex) => {
          const value = Math.max(Number(series.values[labelIndex]) || 0, 0);
          const height = (value / maximum) * chartHeight;
          const y = chartBottom + (cumulative / maximum) * chartHeight;
          const percentage = dayTotal > 0 ? (value / dayTotal) * 100 : 0;
          cumulative += value;
          commands.push(
            `q ${hexToPdfColor(series.color, colors[seriesIndex % colors.length])} rg ${x.toFixed(2)} ${y.toFixed(2)} ${barWidth.toFixed(2)} ${height.toFixed(2)} re f Q`,
          );
          if (percentage >= 6 && height >= 12 && barWidth >= 8) {
            commands.push(
              `q 1 1 1 rg BT /F2 6 Tf ${(x + 1).toFixed(2)} ${(y + height / 2 - 2).toFixed(2)} Td (${Math.round(percentage)}%) Tj ET Q`,
            );
          }
        });
      });

      const labelStep = Math.max(Math.ceil(normalizedLine.labels.length / 7), 1);
      normalizedLine.labels.forEach((label, index) => {
        if (index % labelStep === 0 || index === normalizedLine.labels.length - 1) {
          const x = chartLeft + index * groupWidth + groupWidth / 2;
          commands.push(
            `BT /F1 8 Tf ${(x - 9).toFixed(2)} ${(chartBottom - 10).toFixed(2)} Td (${escapePdfText(formatPdfChartAxisLabel(label))}) Tj ET`,
          );
        }
      });
      normalizedLine.series.forEach((series, seriesIndex) => {
        ...
        const legendRow = Math.floor(seriesIndex / 4);
        const legendX = 62 + legendColumn * 125;
        const legendY = chartBottom - 43 - legendRow * 13;
        commands.push(
          `q ${hexToPdfColor(series.color, colors[seriesIndex % colors.length])} rg ${legendX} ${legendY.toFixed(2)} 8 8 re f Q`,
          `BT /F1 9 Tf ${legendX + 12} ${(legendY + 1).toFixed(2)} Td (${escapePdfText(series.label)}) Tj ET`,
        );
      });

      commands.push(
        `BT /F2 8 Tf 270 ${(chartBottom - 24).toFixed(2)} Td (${escapePdfText(normalizedLine.xAxisLabel)}) Tj ET`,
        `BT /F2 8 Tf 0 1 -1 0 30 ${(chartBottom + 42).toFixed(2)} Tm (${escapePdfText(normalizedLine.yAxisLabel)}) Tj ET`,
      );

      currentY = chartBottom - 51 - Math.ceil(normalizedLine.series.length / 4) * 13 - normalizedLine.spaceAfter;
      return;
    }
