...
function getPdfLineHeight(line) {
  ...
  if (line.type === "lineChart") {
    return line.spaceBefore + 205 + Math.ceil(line.series.length / 4) * 12 + line.spaceAfter;
  }

  if (line.type === "stackedBarChart") {
    return line.spaceBefore + 225 + Math.ceil(line.series.length / 4) * 13 + line.spaceAfter;
  }
  ...
  bodyLines.forEach((line) => {
    ...
    if (normalizedLine.type === "lineChart") {
      ...
      normalizedLine.series.forEach((series, seriesIndex) => {
        ...
        const legendY = chartBottom - 18 - legendRow * 12;
        commands.push(
      ...
      normalizedLine.labels.forEach((label, index) => {
        if (index % labelStep === 0 || index === normalizedLine.labels.length - 1) {
          commands.push(
            `BT /F1 8 Tf ${(pointX(index) - 9).toFixed(2)} ${(chartBottom - 8).toFixed(2)} Td (${escapePdfText(label.slice(5))}) Tj ET`,
          );
        }
      });

      currentY = chartBottom - 25 - Math.ceil(normalizedLine.series.length / 4) * 12 - normalizedLine.spaceAfter;
      return;
    }
    ...
    if (normalizedLine.type === "stackedBarChart") {
      ...
      normalizedLine.labels.forEach((label, index) => {
        if (index % labelStep === 0 || index === normalizedLine.labels.length - 1) {
          ...
          commands.push(
            `BT /F1 8 Tf ${(x - 9).toFixed(2)} ${(chartBottom - 10).toFixed(2)} Td (${escapePdfText(label.slice(5))}) Tj ET`,
          );
        }
      });
      normalizedLine.series.forEach((series, seriesIndex) => {
        ...
        const legendRow = Math.floor(seriesIndex / 4);
        const legendX = 62 + legendColumn * 125;
        const legendY = chartBottom - 25 - legendRow * 13;
        commands.push(
      ...
      currentY = chartBottom - 33 - Math.ceil(normalizedLine.series.length / 4) * 13 - normalizedLine.spaceAfter;
      return;
    }
