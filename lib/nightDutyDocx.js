...
function buildLineChartSvg(line) {
  ...
  const grid = [0, 0.25, 0.5, 0.75, 1].map((ratio) => {
    ...
    return `<line x1="${left}" y1="${y}" x2="${width - right}" y2="${y}" stroke="#D5DEE8" stroke-width="0.75" stroke-dasharray="3 5"/><text x="${left - 7}" y="${y + 3}" text-anchor="end" font-family="Calibri,Arial" font-size="9" fill="#64748B">${escapeXml(compactChartValue(maximum * ratio))}</text>`;
  }).join("");
  ...
  const axisLabels = labels.map((label, index) => (
    index % labelStep === 0 || index === labels.length - 1
      ? `<text x="${pointX(index)}" y="${height - 14}" text-anchor="middle" font-family="Calibri,Arial" font-size="9" fill="#64748B">${escapeXml(asText(label).slice(5))}</text>`
      : ""
  )).join("");

  return `<svg xmlns="http://www.w3.org/2000/svg" width="${width}" height="${height}" viewBox="0 0 ${width} ${height}"><rect width="${width}" height="${height}" fill="#FBFCFE"/><rect x="${left}" y="${top}" width="${plotWidth}" height="${plotHeight}" fill="#FFFFFF" stroke="#D5DEE8" stroke-width="0.7"/>${grid}${paths}${axisLabels}</svg>`;
}
  ...
  return [
    ...
    new Paragraph({
      ...
      children: [new ImageRun({
        ...
        transformation: { width: 624, height: 199 },
...
function buildGroupedBarChartSvg(line) {
  const labels = Array.isArray(line.labels) ? line.labels : [];
  const series = Array.isArray(line.series) ? line.series : [];
  const width = 720;
  const height = 240;
  ...
  const barWidth = Math.max(
    Math.min((groupWidth * 0.78) / Math.max(series.length, 1) - gap, 16),
    1.5,
  );
  const grid = [0, 0.25, 0.5, 0.75, 1].map((ratio) => {
    const y = top + plotHeight - ratio * plotHeight;
  ...
  const bars = labels.map((_, labelIndex) => {
    const groupStart = left + labelIndex * groupWidth + groupWidth * 0.11;
    return series.map((entry, seriesIndex) => {
      ...
      return `<rect x="${groupStart + seriesIndex * (barWidth + gap)}" y="${top + plotHeight - barHeight}" width="${barWidth}" height="${barHeight}" rx="1" fill="#${color}"/>`;
    }).join("");
  }).join("");
  const labelStep = Math.max(Math.ceil(labels.length / 8), 1);
  const axisLabels = labels.map((label, index) => (
    index % labelStep === 0 || index === labels.length - 1
      ? `<text x="${left + index * groupWidth + groupWidth / 2}" y="${height - 15}" text-anchor="middle" font-family="Calibri,Arial" font-size="9" fill="#64748B">${escapeXml(asText(label).slice(5))}</text>`
      : ""
  )).join("");

  return `<svg xmlns="http://www.w3.org/2000/svg" width="${width}" height="${height}" viewBox="0 0 ${width} ${height}"><rect width="${width}" height="${height}" fill="#FBFCFE"/><rect x="${left}" y="${top}" width="${plotWidth}" height="${plotHeight}" fill="#FFFFFF" stroke="#D5DEE8" stroke-width="0.7"/>${grid}${bars}${axisLabels}</svg>`;
}

function buildGroupedBarChart(line) {
  const series = Array.isArray(line.series) ? line.series : [];
  const labels = Array.isArray(line.labels) ? line.labels : [];
  if (series.length === 0 || labels.length === 0) return [];
  const legendRuns = series.flatMap((entry, index) => {
  ...
  return [
    ...
    new Paragraph({
      ...
      children: [new ImageRun({
        type: "svg",
        data: new TextEncoder().encode(buildGroupedBarChartSvg(line)),
        transformation: { width: 624, height: 208 },
        fallback: { type: "png", data: TRANSPARENT_PNG },
      })],
    }),
    new Paragraph({
...
function lineToChildren(line) {
  ...
  if (line?.type === "groupedBarChart") return buildGroupedBarChart(line);
...
export async function createNightDutyRangeDocxBlob({ title, rangeLabel, lines }) {
  const document = new Document({
    ...
    styles: {
      default: {
        document: {
          run: { font: "Calibri", size: 24, color: COLORS.slate },
