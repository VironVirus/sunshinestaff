...
function buildLineChartSvg(line) {
  ...
  const grid = [0, 0.25, 0.5, 0.75, 1].map((ratio) => {
    ...
    return `<line x1="${left}" y1="${y}" x2="${width - right}" y2="${y}" stroke="#D5DEE8" stroke-width="0.75" stroke-dasharray="3 5"/><text x="${left - 7}" y="${y + 3}" text-anchor="end" font-family="Calibri,Arial" font-size="8" fill="#64748B">${escapeXml(compactChartValue(maximum * ratio))}</text>`;
  }).join("");
  ...
  const axisLabels = labels.map((label, index) => (
    index % labelStep === 0 || index === labels.length - 1
      ? `<text x="${pointX(index)}" y="${height - 14}" text-anchor="middle" font-family="Calibri,Arial" font-size="8" fill="#64748B">${escapeXml(asText(label).slice(5))}</text>`
      : ""
  )).join("");
...
function buildLineChart(line) {
  ...
  const legendRuns = series.flatMap((entry, index) => {
    ...
    return [
      new TextRun({ text: index === 0 ? "" : "    ", font: "Calibri", size: 16 }),
      new TextRun({ text: "━ ", bold: true, color, font: "Calibri", size: 18 }),
      new TextRun({ text: asText(entry.label), color: COLORS.slate, font: "Calibri", size: 16 }),
    ];
  });
...
function lineToChildren(line) {
  ...
  return [new Paragraph({
    ...
    children: [new TextRun({
      ...
      font: "Calibri",
      size: Math.round((Number(entry.fontSize) || (isSection ? 13 : 11)) * 2),
    })],
  })];
}
...
export async function createNightDutyRangeDocxBlob({ title, rangeLabel, lines }) {
  const document = new Document({
    ...
    styles: {
      default: {
        document: {
          run: { font: "Calibri", size: 22, color: COLORS.slate },
