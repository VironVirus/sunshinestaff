...
function buildLineChartSvg(line) {
  ...
  const axisLabels = labels.map((label, index) => (
    index % labelStep === 0 || index === labels.length - 1
      ? `<text x="${pointX(index)}" y="${height - 14}" text-anchor="middle" font-family="Calibri,Arial" font-size="10" fill="#64748B">${escapeXml(asText(label).slice(5))}</text>`
      : ""
  )).join("");
...
function buildStackedBarChartSvg(line) {
  ...
  const axisLabels = labels.map((label, index) => (
    index % labelStep === 0 || index === labels.length - 1
      ? `<text x="${left + index * groupWidth + groupWidth / 2}" y="${height - 15}" text-anchor="middle" font-family="Calibri,Arial" font-size="10" fill="#64748B">${escapeXml(asText(label).slice(5))}</text>`
      : ""
  )).join("");
