...
export const operationsMetricConfig = {
  ...
  availableRooms: {
    label: "Available room inventory",
...
export function isOperationsManager(profile) {
  return profile?.staffTitle?.trim().toLocaleLowerCase("en-US") === "operations manager" &&
    isLead(profile);
}

export function canManageOperationsTargets(profile) {
  return isSuperAdmin(profile) || isFrontOfficeManager(profile) || isOperationsManager(profile);
}
...
export function getStoreAccess(profile) {
  ...
  return {
    ...
    canEditPanel: isSuperAdmin(profile) || (isStoreStaff(profile) && isLead(profile)),
  };
}
...
export function getRoomPropertyStatusAccess(profile) {
  return {
    canViewPanel: canAccessManagerWorkspace(profile),
    canEditPanel: isSuperAdmin(profile) || isHousekeepingManager(profile),
    canPrintPanel: canAccessManagerWorkspace(profile),
  };
}

export function getNightDutyAccess(profile) {
  const nightDutyWorkspace = canAccessManagerWorkspace(profile);
  const canEditPanel = isSuperAdmin(profile) || isNightDutyManager(profile);
...
export function buildProfilePayload({
  ...
  return {
    ...
    employmentStartDate: getHotelDateKey(),
    surcharges: "",
    leaveRecords: [],
    monthlySalary: 0,
