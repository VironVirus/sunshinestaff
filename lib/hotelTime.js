...
export function compareDateKeys(leftDateKey, rightDateKey) {
  ...
  if (!left || !right) {
    return String(leftDateKey ?? "").localeCompare(String(rightDateKey ?? ""));
  }
...
export function listDateKeysInRange(startDateKey, endDateKey) {
  ...
  while (currentDateKey && compareDateKeys(currentDateKey, rangeEnd) <= 0) {
    values.push(currentDateKey);
...
export function getHotelHour(value = new Date()) {
  return getHotelParts(value).hour;
}
